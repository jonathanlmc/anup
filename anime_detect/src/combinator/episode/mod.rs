...
pub fn parse_filename(mut filename: &str) -> Result<Parsed> {
    ...
    match (episode_and_season, type_hint) {
        (Ok(ep_and_season), type_hint) => Ok(Parsed {
            ...
            // this is a TV series
            series_type_hint: type_hint.unwrap_or(SeriesTypeHint::TV),
            number: ep_and_season.number,
        }),
        // if we at least have a type hint for a special-like episode, treat this as a one-off
        (Err(_), Some(type_hint)) if type_hint != SeriesTypeHint::TV => Ok(Parsed {
            season_hint: Some(0),
...
fn type_hint_label(input: &mut &str) -> Result<SeriesTypeHint> {
    (
        ...
        opt(any_tag_start),
        alt((
            (Caseless("special"), opt(Caseless("s"))).map(|_| SeriesTypeHint::Special),
            // case-sensitive to avoid false positives
            "ONA".map(|_| SeriesTypeHint::ONA),
            "OVA".map(|_| SeriesTypeHint::OVA),
            Caseless("movie").map(|_| SeriesTypeHint::Movie),
        )),
        opt(file_version),
    )
...
mod tests {
    ...
    mod parse_filename {
        ...
        #[track_caller]
        fn cmp_ep_and_type(input: &str, expected_ep: u32, expected_type: SeriesTypeHint) {
            cmp(
                input,
                Parsed {
                    number: expected_ep,
        ...
        #[test]
        fn with_type_hint() {
            cmp_ep_and_type("Series Title ONA - 12.mkv", 12, SeriesTypeHint::ONA);
            cmp_ep_and_type("Series Title - 12 (ONA).mkv", 12, SeriesTypeHint::ONA);
            cmp_ep_and_type("Series Title OVA - 12.mkv", 12, SeriesTypeHint::OVA);
            cmp_ep_and_type("Series Title Special - 12.mkv", 12, SeriesTypeHint::Special);

            cmp_ep_and_type(
                "Series Title Specials - 12.mkv",
                12,
                SeriesTypeHint::Special,
            );
            ...
            cmp_one_off("Series Title - ONA.mkv", SeriesTypeHint::ONA);
            cmp_one_off("Series Title - ONAv2.mkv", SeriesTypeHint::ONA);
            cmp_one_off("Series Title Movie.mkv", SeriesTypeHint::Movie);
        }
    }
}
